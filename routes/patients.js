...
router.get('/', auth.protegirRuta(["admin", "physio", "patient"]), async (req, res) => {
    try{
        ...
        if(resultat.length > 0){
            ...
            res.status(404).send({error: "No hi ha pacients en el sistema"});
        }
    } catch (error){
        res.status(500).send({error: "Error obtenint pacients"});
    }
});
...
auth.protegirRutaIdPatient(), async (req, res) =>{
    try{
        ...
        if(resultat){
            ...
            res.status(404).send({result: "No hi ha pacients amb aquests criteris de cognom"});
        }
    } catch (error){
        res.status(500).send({error: "Error buscant el pacient indicat"});
    }
...
router.post('/', auth.protegirRuta(["admin", "physio"]), async (req, res) =>{
    try{
        ...
        res.status(400).send({error:"Error al inserir un pacient"});
    }
});
